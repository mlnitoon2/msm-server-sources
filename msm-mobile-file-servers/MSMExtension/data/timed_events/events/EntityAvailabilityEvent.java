...
public class EntityAvailabilityEvent extends TimedEvent {
   ...
   public EntityAvailabilityEvent(ISFSObject timedEventData) throws Exception {
      super(timedEventData);
      if (this.eventDataSfsArray.size() != 1) {
         throw new Exception("EntityAvailabilityEvent has invalid number of entries in data field; can only have 1");
      } else {
         this.entityId = ((SFSObject)this.eventDataSfsArray.getElementAt(0)).getInt("entity");
         if (this.entityId == null) {
            Logger.trace("EntityAvailabilityEvent: entityId is null");
         }
   ...
   public boolean equals(Object o) {
      if (o == this) {
         return true;
      } else if (!(o instanceof EntityAvailabilityEvent)) {
         return false;
      } else {
         EntityAvailabilityEvent e = (EntityAvailabilityEvent)o;
   ...
   public static boolean hasTimedEventNow(Entity entity, Player player, int islandType) {
      return TimedEventManager.instance().hasTimedEventNow(TimedEventType.EntityStoreAvailability, entity.getEntityId(), islandType) || player.getTimedEvents().timedEventNow(TimedEventType.EntityStoreAvailability, entity.getEntityId(), islandType);
   }
}
